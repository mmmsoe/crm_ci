...
class Invoices extends CI_Controller {
	...
	function index($customer_id='')
	{
			...
			if (!check_staff_permission('invoices_read'))	
			{
		    	...
		    	$cek_invoices = $this->invoices_model->invoices_list($customer_id);
		    	
				if( ! empty($cek_invoices) ){
					foreach( $cek_invoices as $cek_invoice){
						if( date('Y-m-d') > date('Y-m-d', $cek_invoice->due_date) ){
							if($cek_invoice->status !== "Paid Invoice"){
								// echo $cek_invoice->id;
								$this->invoices_model->update_invoice_overdue($cek_invoice->id);
							}
						}
					}
				}
	...
	function update($invoice_id)
	{
		//checking permission for staff
			if (!check_staff_permission('invoices_write'))	
			{
		    	...
		    	$data['products'] = $this->products_model->products_list();
		    	$data['qo_products'] = $this->invoices_model->invoice_products($invoice_id);
		    	$data['total_payment'] = $this->invoices_model->get_count_payment_received($invoice_id);
		    	$data['payment_methods'] = $this->system_model->system_list('PAYMENT_METHOD');
	...
	function update_process()
	{
		...
		$this->form_validation->set_rules('payment_received', 'Amount', 'required');
		
		$nominal = $this->input->post('payment_received');
		
		if( $this->form_validation->run() == FALSE )
        {
            echo '<div class="alert error" style="color:red"> <ul><li>' . validation_errors() . '</li></ul></div>';
        }
        else if( $nominal <= 0.00)
        {
            echo '<div class="alert alert-danger">Amount can\'t be 0 (zero)<div>';
        }
        else if( $this->invoices_model->check_unpaid_amount($this->input->post('invoice_id') ) == false)
        {
        ...
        else
        {
            if( $this->invoices_model->update_invoice() )
            {
				...
				{
					echo 'yes_'.$this->input->post('invoice_id');
				}
				else
				{
	...
	function receive_payments_number()
	{
		...
		$this->load->view('header');
		$this->load->view('invoices/receive_payments_by_number',$data);
		$this->load->view('footer');
	...
	function receive_payments_number_process()
	{
        ...
        else
        {
            ...
            {
                echo 'yes_'.$this->input->post('invoice_number');
            }
            else
            {
                echo $this->lang->line('technical_problem');
