...
class Permission_model extends CI_Model {
    ...
    function staff_permission($account_permission_id = '') {
        if ($this->user_model->get_role(userdata('id'))[0]->role_id == '1') {
            ...
            $current_staff_account_role_id = $this->db->get_where('users', array('id' => userdata('id')))
                ->row()->account_role_id;

            $current_staff_account_permissions = $this->db->get_where('account_role', array('id' => $current_staff_account_role_id))
