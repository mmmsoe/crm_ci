  ...
  <div class="main_detail">
    ...
    <div class="detail_view_item">      
      <table width="100%" cellspacing="0" cellpadding="0" border="">
        <tbody>
          <tr>
            <th style="width:200px;"><b>Type</b></th>
            <th><b>Potential Name</b></th>
            <th><b>Account Name</b></th>
            <th><b>Potential Owner</b></th>
...
if ($opportunity_group->type_id !==''){
$html .=' <tr>
	    <td colspan="8" style="background:#f0f4f8">'.$this->system_model->system_single_value('OPPORTUNITIES_TYPE', $opportunity_group->type_id)->system_value_txt .' ('.$opportunity_group->cnt.')'.'</td>
	   </tr>';	
$opportunities = $this->opportunities_model->get_list_by_group( array('type_id' => $opportunity_group->type_id),$min,$max ); 
if( ! empty($opportunities) ){
foreach( $opportunities as $opportunity){
					 
$html .='<tr>
            <td></td>
            <td style="width:20%">'.$opportunity->opportunity.'</td>
            <td style="width:20%">'.$this->customers_model->get_account_name($opportunity->customer_id,'name').'</td>
            <td style="width:20%">'.$this->staff_model->get_user_fullname($opportunity->salesperson_id).'</td>
            <td style="width:15%">'.$opportunity->expected_closing.'</td>
            <td style="width:20%">'.$this->system_model->system_single_value('OPPORTUNITIES_STAGES', $opportunity->stages_id)->system_value_txt.'</td>
			<td style="text-align:right">'.$opportunity->probability.'</td>
     		<td style="width:20%">'.$this->system_model->system_single_value('LEAD', $opportunity->lead_source_id)->system_value_txt.'</td>
			</tr>';
}}
}}
}
