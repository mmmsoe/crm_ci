...
//if($leads_group->industry_id !== '' || $leads_group->customer_id !== '0' || $leads_group->annual_revenue !== '') { 
if ($leads_group->industry_id !=='') {
$html .='  <tr>
				<td colspan="4" style="background:#f0f4f8">'.$this->system_model->system_single_value('INDUSTRY', $leads_group->industry_id)->system_value_txt .' ('.$leads_group->cnt.')'.'</td>
				
		</tr>';
}else{
$html .='<tr>
				<td colspan="4" style="background:#f0f4f8">'. 'No Industry' .' ('.$leads_group->cnt.')'.'</td> ;
				
		</tr>';
}

$leads = $this->leads_model->get_list_by_group_account( array('industry_id' => $leads_group->industry_id), $min, $max ,'' ); 
if( ! empty($leads) ){
foreach( $leads as $leads){ 
						
$html .='<tr>
		    <td></td>
	        <td>'.$this->customers_model->get_account_name($leads->customer_id,'name').'</td>
	        <td>'.$this->system_model->system_single_value('RATING', $leads->rating_id)->system_value_txt.'</td>
	      	<td>'.number_format($leads->annual_revenue,2,'.',',').'</td>
