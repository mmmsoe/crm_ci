...
$html .='
			  <tr>
				<td colspan="6" style="background:#f0f4f8">'.$this->system_model->system_single_value('LEAD', $opportunity_group->lead_source_id)->system_value_txt .' ('.$opportunity_group->cnt.')'.'</td>
			  </tr>';
			  
$opportunities = $this->opportunities_model->get_list_by_group( array('lead_source_id' => $opportunity_group->lead_source_id), $min, $max, 'quotations_salesorder' );
...
$html .='
			  <tr>			  
				...
				<td>'. $this->system_model->system_single_value('OPPORTUNITIES_STAGES', $opportunity_group->stages_id)->system_value_txt .'</td>
				<td class="numeric">'. number_format($opportunity->amount, 2, '.', ',') .'</td>
			  </tr>';
				}
			}
$html .='
			  <tr>
				<td colspan="6" style="border-top: 3px solid #ddd;" class="numeric">'. number_format($opportunity_group->amt, 2, '.', ',') .'</td>
			  </tr>';
	}
}
$html .='
			  <tr>
				<td colspan="6" style="background:#ddd;" class="numeric">'. number_format($sum_amount, 2, '.', ',') .'</td>
			  </tr>';
