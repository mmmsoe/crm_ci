...
if( ! empty($opportunities_group) ){
	foreach( $opportunities_group as $opportunities_group){
		
$html .='
		 <tr>			  
			
			<td>'.$this->system_model->system_single_value('OPPORTUNITIES_STAGES', $opportunities_group->stages_id)->system_value_txt .' ('.$opportunities_group->cnt.')'.'</td>
			<td class="numeric">'.number_format($opportunities_group->existing_bussines,2,'.',',').'</td>
			<td class="numeric">'.number_format($opportunities_group->new_bussines,2,'.',',').'</td>
			<td class="numeric">'.number_format($opportunities_group->amt,2,'.',',').'</td>
	    </tr>';
				}
			}
$html .='
		<tr>
			...
			<td style="background:#ddd;" class="numeric">'.number_format($typetwo, 2, '.', ',').'</td>
		    <td  style="background:#ddd;" class="numeric">'. number_format($typeone, 2, '.', ',').'</td>
	        <td  style="background:#ddd;" class="numeric">'. number_format($sum_amount, 2, '.', ',').'</td>
		</tr>';
