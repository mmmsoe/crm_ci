 <?php
	...
	if($list[0]->lead_sources){
		...
		$ind_name = explode("~", $list[0]->regis_datetime); //$list[0] adalah index header
		for ($i = 0; $i < count($ind_name); $i++) {
			echo "<td class='numeric'>" . $this->report_model->get_avg_sales_duration_lead_source_by_month($ind_name[$i],$min,$max) . "</td>";
		}
		echo "<td class='numeric'>" . $this->report_model->get_avg_sales_duration_lead_source_by_month('',$min,$max) . "</td>";
		echo "</tr>";
	}
		echo "</tbody>";
    ?>