...
$html .="<th>" . $list[0]->potential_type . "</th>";
...
$html .="<td>". $list[$i]->potential_type  ."</td>";
...
$html .="<td><b>Avg of Overall Sales Duration in Day(s)</b></td>";
		$ind_name = explode("~", $list[0]->regis_datetime); //$list[0] adalah index header
		for ($i = 0; $i < count($ind_name); $i++) {
$html .="<td class='numeric'>" . $this->report_model->get_avg_sales_duration_potential_type_by_month($ind_name[$i],$min,$max) . "</td>";
		}
$html .="<td class='numeric'>" . $this->report_model->get_avg_sales_duration_potential_type_by_month('',$min,$max) . "</td>";
$html .="</tr>";
