...
$(document).ready(function() {
	...
	$("#print").attr('href', '<?php echo base_url('admin/report/print_lead_conversion_accross_owner	').'/'; ?>');
			
	$("#applyFilter").click(function() {
		var min = document.getElementById("min").value;
		var max = document.getElementById("max").value;
		var stat = 'accross_owner';
		...
		$.ajax({
			...
			success: function(data){
				...
				$("#print").attr('href', '<?php echo base_url('admin/report/print_lead_conversion_accross_owner').'/'; ?>'+ mindt +'/'+ '-' +'/'+ maxdt);
			},
		});
	});
});
...
.table-hover > tbody > tr.nohov:hover > td {
 ...
 <!-- BEGIN PAGE CONTENT -->
        <div class="page-content">
			...
			echo "<th style='width: 68px;'>Avg of Lead Conversion Time in Day(s)</th>";
			echo "</tr>";
			echo "</thead>";
			//end of table header
			...
			for ($i = 1; $i < count($list); $i++) {
				...
				$ind_name = explode("~", $list[0]->regis_datetime);
				for ($i = 0; $i < count($ind_name); $i++) {
				 echo "<td class='numeric'>" . $this->report_model->get_avg_conversion_accross_owner($ind_name[$i],'','') . "</td>";
					}
				echo "<td class='numeric'>" . $this->report_model->get_avg_conversion_accross_owner() . "</td>";
				echo "</tr>";
				echo "</tbody>";
			?>
