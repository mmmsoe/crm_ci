...
if( ! empty($opportunities_group) ){
	foreach( $opportunities_group as $opportunity_group){
		if($opportunity_group->stages_id !== '0' && $opportunity_group->stages_id !== '') {
$html .='
			  <tr>
				<td colspan="6" style="background:#f0f4f8">'.$this->system_model->system_single_value('OPPORTUNITIES_STAGES', $opportunity_group->stages_id)->system_value_txt .' ('.$opportunity_group->cnt.')'.'</td>
			  </tr>';
			  
$opportunities = $this->opportunities_model->get_list_by_group( array('stages_id' => $opportunity_group->stages_id),$min,$max,'stages');
...
$html .='
			  <tr>			  
				...
				<td class="numeric">'. $opportunity->probability .'</td>
