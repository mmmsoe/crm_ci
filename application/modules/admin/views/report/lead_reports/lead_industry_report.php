...
$(document).ready(function() {
	...
	$("#print").attr('href', '<?php echo base_url('admin/report/print_lead_industry').'/'; ?>');
			
	$("#applyFilter").click(function() {
		var min = document.getElementById("min").value;
		var max = document.getElementById("max").value;
		var stat = 'lead_industry';
		...
		$.ajax({
			...
			success: function(data){
				...
				$("#print").attr('href', '<?php echo base_url('admin/report/print_lead_industry').'/'; ?>'+ mindt +'/'+ '-' +'/'+ maxdt);
			},
		});
	});
});
                     ...
                     <tbody>
                      ...
                      <?php if( ! empty($leads_group) ){?>
					    <?php foreach( $leads_group as $lead_group){ ?>
						  ...
						  <?php $leads = $this->leads_model->get_list_by_group_account( array('a.industry_id' => $lead_group->industry_id),'','',''); ?>
						  <?php if( ! empty($leads) ){?>
