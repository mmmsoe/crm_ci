...
$html .='
			  <tr>
				<td colspan="6" style="background:#f0f4f8">'. $lead_group->lead_name .' ('.$lead_group->cnt.')' .'</td>
			  </tr>';
			  
$leads = $this->leads_model->get_list_by_group_account( array('a.id' => $lead_group->id),$min,$max,'converted');
			if( ! empty($leads) ){
...
$html .='
			  <tr>			  
				...
				<td>'. $this->staff_model->get_user($lead->salesperson_id)->first_name." ".$this->staff_model->get_user($lead->salesperson_id)->last_name .'</td>
			  </tr>';
				}
			}
