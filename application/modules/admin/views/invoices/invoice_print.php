...
<div class="main" style="margin-top:-90px !important">  
  <div class="main_detail">
    <div class="detail_head_titel" style="margin-top:-30px !important">Sales Invoice<br><span><b>Ref No : </b>'.$invoice->invoice_number.'</span> </div>
    <div class="detail_view_item" style="margin-bottom:0px;">
	  <div class="view_frist">
		<span><b>Sold To :</b></span><br>
		<div style="line-height:15px;margin-bottom:10px;"><b>'.$this->customers_model->get_company($salesorder->customer_id)->name.'</b>
		<br>'.$this->customers_model->get_company($salesorder->customer_id)->address.'</div>
		<div style="width:150px; float:left;margin-bottom:10px;">
		<span><b>Tel :</b> '.$this->customers_model->get_company($salesorder->customer_id)->phone.'</span>
		</div>
		<div style="width:150px; float:left;margin-bottom:10px;">
		<span><b>Fax :</b> '.$this->customers_model->get_company($salesorder->customer_id)->fax.'</span>
		</div>
	  </div>
    </div>
    <div class="detail_view_customer" style="margin-bottom:10px;">
      <table width="100%" cellspacing="0" cellpadding="0" border="">
        <tbody>
          ...
          <tr>
            <td>'.$this->contact_persons_model->get_contact_persons($salesorder->contact_id)->first_name .' '. $this->contact_persons_model->get_contact_persons($salesorder->contact_id)->last_name .'</td>
            <td>'. $term .'</td>
            <td>'. $this->staff_model->get_user_fullname($salesorder->sales_person) .'</td>
    ...
    <div class="detail_view_item" style="margin-top:10px;">
	  <div style="width:150px; float:left; line-height:15px;">
		<span><b>Invoice Date:</b><br>'.date('m/d/Y',$invoice->invoice_date).'</span>
	  </div>
	  <div style="width:150px; float:left; line-height:15px;">
		<span><b>Due Date:</b><br>'.date('m/d/Y',$invoice->due_date).'</span>
	  </div>
    </div>
...
$html .='<tr>
            ...
            <td><b>'.$qo_product->product_name.'</b><br><pre style= "border: medium none; background: none; padding: 0px; margin: 0px;">'.$qo_product->discription.'</pre></td>
            <td style="text-align:center;">'.$qo_product->quantity.'</td>
            <td style="">'.$uom.'</td>
            <td style="text-align:right;">'.number_format($qo_product->price, 2, '.', ',').'</td>
            <td style="text-align:right;">'.$this->invoices_model->get_qo_product($qo_product->product_id)->product_discount.'</td>
            <td style="text-align:right;">'.number_format($qo_product->quantity*$qo_product->price*$this->products_model->get_products($product_id)->tax/100,2,'.',',').'</td>
...
$html .=' </tbody>
    ...
    <div class="detail_view_item">
      <table width="100%" cellspacing="0" cellpadding="0" border="" class="fl_right;">
        <tbody>
          <tr>
            <td colspan="2" rowspan="2" width=""><b>Remarks :</b><br>'.$invoice->terms_and_conditions.'</td>
            <td width="15%"><b>SubTotal</b></td>
            <td style="width:20%;text-align:right;">'.number_format($invoice->total, 2, '.', ',').'</td>
          </tr>
          <tr>
            <td><b>Discount</b></td>
          ...
          <tr>
          	<td colspan="3"><b>GST In Base Currency</b></td>
            <td style="text-align:right;">'.number_format($invoice->tax_amount, 2, '.', ',').'</td>
          </tr>
          <tr>
          	<td colspan="3"><b>Net Amount</b></td>
            <td style="text-align:right;" id="grand_total">'.number_format($invoice->grand_total, 2, '.', ',').'</td>
          </tr>
          <tr>
          	<td colspan="4">Amt in word : <span id="word_total">'.$this->number_towords->convert_number_to_words($invoice->grand_total).'</span></td>
          </tr>
        </tbody>
      </table>
    </div>
...
$company_name = $this->customers_model->get_company($salesorder->customer_id)->name; 
$mpdf->SetTitle($company_name." - Invoice");
$mpdf->SetAuthor("Acme Trading Co.");
//$mpdf->SetWatermarkText($salesorder->payment_term);
