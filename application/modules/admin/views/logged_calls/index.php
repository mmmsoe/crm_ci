...
<!--h2 class="col-md-6"><strong>Logged Calls</strong></h2--> 
        <div>
            ...
            <?php if (check_staff_permission('logged_calls_write')) { ?> <a style="float:right;" href="<?php echo base_url('admin/logged_calls/add/'); ?>" class="btn btn-primary btn-embossed"> Create New</a>  <?php } ?>
            <?php if (check_staff_permission('logged_calls_read')) { ?><a href="<?php echo base_url('admin/logged_calls'); ?>" class="btn btn-black btn-embossed"> Logged Calls</a><?php } ?>
            ...
            <a href="<?php echo base_url('admin/mail_entry'); ?>" class="btn btn-gray btn-embossed"> E-Mail</a>
                            ...
                            <?php if (!empty($logged_calls)) { ?>
                                <?php foreach ($logged_calls as $logged_call) { ?>
                                    <tr id="logged_call_id_<?php echo $logged_call->id; ?>">
                                        ...
                                        <td><?php echo date('m/d/Y H:i', $logged_call->date); ?></td>         
