    ...
    <div class="row">
        <div>
            <?php if (check_staff_permission('sales_orders_write')) { ?>
                ...
                <a  style="float:right;" href="<?php echo base_url('admin/salesorder'); ?>" class="btn btn-success btn-embossed">Sales Order</a>
                <a href="<?php echo base_url('admin/salesorder/'); ?>" class="btn btn-black btn-embossed"> Manage Sales Order</a> 	
            <?php } ?>
        </div>           
    </div>
    ...
    <div class="row">
        <div class="panel">																				<div class="panel-content">
                        ...
                        <tbody>
                            ...
                            <?php if (!empty($salesorder)) { ?>
                                <?php foreach ($salesorder as $order) { ?>
                                    <tr id="order_id_<?php echo $order->quotations_id; ?>">

                                        <td><a href="<?php echo base_url('admin/salesorder/view/' . $order->quotations_id); ?>"><?php echo $order->quotations_number; ?></a></td>	                         		<td><?php echo date('m/d/Y H:i', $order->date); ?></td>
                                        <td><?php echo customer_name($order->customer_id)->name; ?></td>
                                        <td><?php echo $this->staff_model->get_user_fullname($order->sales_person); ?></td>

                                        <td class="numeric"><?php echo number_format($order->grand_total, 2, '.', ','); ?></td>

                                        <td><?php echo $order->status; ?></td>

                                        <td style="width: 12%;">
                                            <?php if (check_staff_permission('sales_orders_write')) { ?>
                                                <a href="<?php echo base_url('admin/salesorder/update/' . $order->quotations_id); ?>" class="edit btn btn-sm btn-default dlt_sm_table"><i class="icon-note"></i></a> 
                                            <?php } ?>
             ...
             <!--a href="javascript:void(0)" class="delete btn btn-sm btn-danger dlt_sm_table" data-toggle="modal" data-target="#modal-basic<?php //echo $order->id; ?>"><i class="glyphicon glyphicon-trash"></i></a-->
                                ...
                                <div class="modal fade" id="modal-basic<?php echo $order->quotations_id; ?>" tabindex="-1" role="dialog" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="button" onclick="delete_order(<?php echo $order->quotations_id; ?>)" class="btn btn-primary btn-embossed" data-dismiss="modal">Delete</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
