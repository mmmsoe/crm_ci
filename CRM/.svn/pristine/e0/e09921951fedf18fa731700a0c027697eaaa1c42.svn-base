...
class Invoices_model extends CI_Model {
    ...
    function get_count_payment_received( $invoice_id )
	{
		$this->db->select('SUM(payment_received) as payrec');
		$this->db->from('invoice_receive_payments');
		$this->db->where(array('invoice_id' => $invoice_id));
		return $this->db->get()->row()->payrec;
	}
	      ...
	      //Add or edit quotation template products 
				...
				for($i=0;$i<$count;$i++)
				{
					...
					if(isset($this->input->post('quotation_product_id')[$i])){
						...
						$this->db->update('invoices_products',$products_edit,array('id' => $this->input->post('quotation_product_id')[$i]));			  
					}
				}
	...
	function update_invoice_overdue($invoice_id)
    {
		 ...
		 return $this->db->update('invoices',$invoice_details,array('id' => $invoice_id));
	...
	function get_invoice_by_order( $order_id )
	{
	...
	{
		return $this->db->get_where('salesorders',array('id' => $order_id))->row();
	}
    ...
    {
		if( $this->input->post('invoice_type') == 'update_invoice_payment' ){
