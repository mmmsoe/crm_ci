...
$html .='
			  <tr>
				<td colspan="6" style="background:#f0f4f8">'. $this->staff_model->get_user_fullname($lead_group->salesperson_id) .' ('.$lead_group->cnt.')' .'</td>
			  </tr>';
			  
$leads = $this->leads_model->get_list_by_group_account( array('a.salesperson_id' => $lead_group->salesperson_id),$min,$max,'');
			if( ! empty($leads) ){
